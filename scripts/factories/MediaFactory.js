import { gaucheRecule, droiteAvance, conserverMiniatureVideo, chargement, garderPrenom, remplaceVideoImage, urlImageMiniature } from '../utils/functions.js'
...
export class MediaFactory {
  ...
  getPhotoDOM () {
    ...
    const source = conserverMiniatureVideo(this.video, picturePhoto, `${filtreLien}_m.jpg`)
    ...
    divClickLike.addEventListener('click', () => {
      ...
      if (clique) { divLikes.innerHTML = `${this.likes + 1} `; divClickLike.innerHTML = "<i class='fa-solid fa-heart fa-heart-margin'></i>" } else {
        divLikes.innerHTML = `${this.likes} `
        divClickLike.innerHTML = "<i class='fa-regular fa-heart fa-heart-margin'></i>"
      }
    }
